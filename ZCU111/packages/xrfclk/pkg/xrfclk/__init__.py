...
For simple (safe) use, refer to getFreqList() and setAllRefClks().

Expert use:
If you need to specify custom clock frequencies AND you know what you're doing(!)
call _writeLmk04208Regs() and _writeLmx2594Regs() with custom register values.
...
def _writeLmk04208Regs(regVals):
    """Write values to the LMK04208 registers.
    This is an internal function --- here be dragons.

    regVals: list of 26 32bit register values
    """
    _safe_wrapper("writeLmk04208Regs", _iic_channel, regVals)

def _writeLmx2594Regs(regVals):
    """Write values to the LMX2594 registers.
    This is an internal function --- here be dragons.

    regVals: list of 113 32bit register values
    """
    _safe_wrapper("writeLmx2594Regs", _iic_channel, regVals)
